...
class FlowDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # validation dataset (with explicit flow ground truth for sintel_split_sep/sintel_eval)
        if self.is_test:
...
class MpiSintel(FlowDataset):
    def __init__(self, aug_params=None, split='training', root='/data/azin/sintel', dstype='clean', occlusion=False, segmentation=False):
...
class FlyingThings3D(FlowDataset):
    def __init__(self, aug_params=None, root='/data/azin/fth', split='training', dstype='frames_cleanpass'):
...
def fetch_dataloader(args, phase, TRAIN_DS='C+T+K+Ssep+H'):
    ...
    if args["train"]["dataset"][phase] == 'chairs':
        ...
        if TRAIN_DS == 'C+T+K+S+H':
            ...
            print("************************\n Mixed Training: C+T+S \n *********************")
    
    # elif args["train"]["dataset"][phase] == 'sintel_split_sep':
    #     aug_params = {'crop_size': args["train"]["image_size"][phase], 'min_scale': -0.2, 'max_scale': 0.6, 'do_flip': True}
    #     things = FlyingThings3D(aug_params, dstype='frames_cleanpass')
    #     sintel_clean = MpiSintelSplitSep(aug_params, split='sintel_train', dstype='clean')
    #     sintel_final = MpiSintelSplitSep(aug_params, split='sintel_train', dstype='final')        

    #     if TRAIN_DS == 'C+T+K+Ssep+H':
    #         kitti = KITTI({'crop_size': args["train"]["image_size"][phase], 'min_scale': -0.3, 'max_scale': 0.5, 'do_flip': True})
    #         hd1k = HD1K({'crop_size': args["train"]["image_size"][phase], 'min_scale': -0.5, 'max_scale': 0.2, 'do_flip': True})
    #         train_dataset = 100*sintel_clean + 100*sintel_final + 200*kitti + 5*hd1k + things
    #         print("************************\n Mixed Training: C+T+K+Ssep+H \n************************")

    #     elif TRAIN_DS == 'C+T+Ssep':
    #         train_dataset = 100*sintel_clean + 100*sintel_final + things
    #         print("************************\n Mixed Training: C+T+Ssep \n *********************")
